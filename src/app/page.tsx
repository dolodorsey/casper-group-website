...
const Grain = () => (
  <div style={{ position: "fixed", inset: 0, zIndex: 9999, pointerEvents: "none", mixBlendMode: "overlay", opacity: 0.03 }}>
    <svg width="100%" height="100%"><filter id="g"><feTurbulence baseFrequency="0.6" numOctaves="3" stitchTiles="stitch" /></filter><rect width="100%" height="100%" filter="url(#g)" /></svg>
  </div>
);
  ...
  return (
    <section style={{ height: "100vh", position: "relative", overflow: "hidden", background: C.base }}>
      ...
      <div style={{ position: "absolute", bottom: "15vh", left: "6vw", zIndex: 3 }}>
        <div style={{
          fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.5em",
          ...
          Est. 2020 — Multi-Concept Dining
        </div>

        <h1 style={{
          fontFamily: "'Playfair Display', serif",
          fontSize: "clamp(52px, 12vw, 180px)", fontWeight: 400,
          lineHeight: 0.88, letterSpacing: "-0.03em", color: C.cream, margin: 0,
        }}>
          ...
          <span style={{ display: "block", fontStyle: "italic", fontWeight: 400, opacity: ready ? 1 : 0, transform: ready ? "translateY(0)" : "translateY(100%)", transition: "all 1.2s cubic-bezier(0.16, 1, 0.3, 1) 0.45s" }}>
            <span style={{ color: C.gold }}>Group</span>
          </span>
        </h1>
        ...
        <div style={{ marginTop: 36, marginLeft: "clamp(60px, 10vw, 160px)", opacity: ready ? 1 : 0, transform: ready ? "translateY(0)" : "translateY(20px)", transition: "all 0.8s ease 0.7s" }}>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "clamp(13px, 1.1vw, 16px)", fontWeight: 300, color: C.silver, lineHeight: 1.6, maxWidth: 320 }}>
            A multi-concept restaurant empire. Every brand in the Casper universe tells its own story.
          </p>
        </div>
      </div>

      <div style={{ position: "absolute", right: "6vw", top: "50%", transform: "translateY(-50%) rotate(90deg)", fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.4em", textTransform: "uppercase", color: C.dim, opacity: ready ? 0.25 : 0, transition: "opacity 1.5s ease 1s", whiteSpace: "nowrap" }}>
        Worldwide →
      </div>
    </section>
  );
...
// ═══════════════════════════════════════════════════════════════════
// CONCEPT WALL — THE SIGNATURE INTERACTION
// ═══════════════════════════════════════════════════════════════════
function ConceptWall() {
  const [expanded, setExpanded] = useState<number | null>(null);

  const concepts = [
    { name: "STUSH", type: "Fine Dining", desc: "Caribbean fine dining meets global technique. Elevated flavors, impeccable service, chef-driven menus.", color: C.burgundy, accent: C.gold },
    { name: "BODEGEA", type: "Retail + Café", desc: "Premium corner store meets curated lifestyle retail. Specialty products, fresh food, community hub.", color: "#2A3540", accent: "#C82424" },
    { name: "SOS", type: "Fast Casual", desc: "Soul food remixed for speed. Honest ingredients, generous portions, no pretense.", color: "#1A2818", accent: "#7AB648" },
    { name: "Beauty & Beast", type: "Bar + Kitchen", desc: "Cocktail-forward lounge with bold small plates. Where nightlife meets real food.", color: "#2A1028", accent: "#D4A5E0" },
  ];
  ...
  return (
    <section style={{ minHeight: "100vh", background: C.deep, padding: "120px 6vw", position: "relative" }}>
      ...
      <R delay={0.1}><h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(40px, 6vw, 80px)", fontWeight: 400, lineHeight: 0.95, color: C.cream, margin: "0 0 72px", letterSpacing: "-0.02em" }}>
        One empire.<br /><em>Four universes.</em>
      </h2></R>

      <div style={{ display: "grid", gridTemplateColumns: expanded !== null ? "1fr" : "repeat(2, 1fr)", gap: 3, transition: "all 0.6s ease" }}>
        {concepts.map((c, i) => (
          <R key={c.name} delay={0.1 + i * 0.06}>
            <div
              onClick={() => setExpanded(expanded === i ? null : i)}
              style={{
                background: expanded === i ? c.color : C.steel,
                padding: expanded === i ? "clamp(48px, 5vw, 80px)" : "clamp(36px, 3vw, 56px)",
                cursor: "pointer",
                position: "relative", overflow: "hidden",
                transition: "all 0.6s cubic-bezier(0.16, 1, 0.3, 1)",
                border: `1px solid ${expanded === i ? c.accent + "30" : C.steel}`,
                minHeight: expanded === i ? 300 : "auto",
              }}
            >
              {/* Ghost number */}
              <div style={{
                position: "absolute", top: 12, right: 20,
                fontFamily: "'Playfair Display', serif", fontSize: "clamp(60px, 6vw, 100px)",
                color: expanded === i ? c.accent : C.base, opacity: expanded === i ? 0.15 : 0.08,
                transition: "all 0.5s ease", lineHeight: 1, fontStyle: "italic",
              }}>0{i + 1}</div>
              ...
              <div style={{
                ...
                marginBottom: 12, transition: "color 0.4s ease",
              }}>{c.type}</div>

              {/* Name */}
              <h3 style={{
                fontFamily: "'Playfair Display', serif",
                fontSize: expanded === i ? "clamp(36px, 4vw, 56px)" : "clamp(24px, 2.5vw, 36px)",
                fontWeight: 500, color: C.cream, margin: "0 0 16px",
              ...
              <div style={{
                ...
                <p style={{
                  ...
                  lineHeight: 1.7, color: C.cream, opacity: 0.6, maxWidth: 500, marginBottom: 28,
                }}>{c.desc}</p>
                <span style={{
                  fontFamily: "'DM Mono', monospace", fontSize: 10, letterSpacing: "0.2em",
                  textTransform: "uppercase", color: c.accent,
                  borderBottom: `1px solid ${c.accent}40`, paddingBottom: 2,
                }}>Explore →</span>
              </div>
              ...
              <div style={{
                ...
                color: C.dim, opacity: expanded === i ? 0 : 0.3,
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "22px 6vw", display: "flex", justifyContent: "space-between", alignItems: "center", background: s ? `${C.base}F2` : "transparent", backdropFilter: s ? "blur(24px)" : "none", borderBottom: s ? `1px solid ${C.steel}` : "1px solid transparent", transition: "all 0.5s ease" }}>
      ...
      <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
        {["Concepts", "Cities", "Careers"].map(i => <a key={i} href="#" style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: C.dim, textDecoration: "none", transition: "color 0.3s ease" }} onMouseEnter={(e) => { (e.target as HTMLElement).style.color = C.cream; }} onMouseLeave={(e) => { (e.target as HTMLElement).style.color = C.dim; }}>{i}</a>)}
    ...
    <footer style={{ background: C.base, padding: "56px 6vw 40px", borderTop: `1px solid ${C.steel}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          ...
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: C.dim, opacity: 0.3 }}>© 2026 Casper Group — A Kollective Hospitality Group Brand</div>
        </div>
