  ...
  return (
    ...
    >
      ...
      <section className="cbh-hero">
        ...
        <div className="cbh-hero-content">
          <div className="cbh-kicker">{experience.shortLabel} · A Casper Group brand</div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img className="cbh-hero-logo" src={profile.logo} alt={profile.name} />
          <p>{profile.description}</p>
          <div className="cbh-actions">
            <Link className="cbh-button cbh-button-primary" href={`/${profile.slug}/menu`}>Explore the menu</Link>
            <Link className="cbh-button" href={`/${profile.slug}/order`}>Start an order</Link>
          </div>
        </div>
        {experience.mascot ? (
          <div className="cbh-mascot" aria-hidden="true">
            {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <section className="cbh-directory" aria-labelledby={`${profile.slug}-directory`}>
        ...
        <div className="cbh-page-grid">
          {PAGE_LINKS.map((page, index) => (
            <Link className="cbh-page-card" href={`/${profile.slug}/${page.key}`} key={page.key}>
              <span className="cbh-page-number">0{index + 1}</span>
              <div>
                <h2>{page.key === 'catering' ? profile.serviceLabel : page.key === 'rewards' ? profile.clubLabel : page.title}</h2>
                <p>{page.copy}</p>
              </div>
              <span className="cbh-arrow">↗</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="cbh-media-deck" aria-label={`${profile.name} visual world`}>
        {experience.secondaryVideo ? (
          <div className="cbh-media cbh-media-video">
            <video autoPlay muted loop playsInline poster={experience.gallery[0]}>
              <source src={experience.secondaryVideo} type="video/mp4" />
            </video>
          </div>
        ) : null}
        {experience.gallery.map((src, index) => (
          <div className="cbh-media" key={src}>
            <Image src={src} alt={`${profile.name} visual ${index + 1}`} fill sizes="(max-width: 900px) 100vw, 33vw" />
          </div>
        ))}
      </section>
      ...
      <footer className="cbh-footer">
        ...
        <div className="cbh-footer-links">
          <Link href="/brands">Casper brands</Link>
          <Link href={`/${profile.slug}/contact`}>Contact</Link>
